...
describe("Delta", () => {
  ...
  describe("<Delta>", () => {
    it("should handle cases with empty values", () => {
      ...
      expect(emptyCurrent.find("span").hasClass("delta-neutral")).to.equal(true);
      expect(emptyCurrent.find("span").contains("-")).to.equal(true);

      expect(emptyPrevious.find("span").hasClass("delta-neutral")).to.equal(true);
      expect(emptyPrevious.find("span").contains("-")).to.equal(true);
    });
    ...
    it("should render properly positive delta", () => {
      ...
      expect(deltaNode.hasClass("delta-positive")).to.equal(true);
    ...
    it("should render properly positive delta for lower-is-better measure", () => {
      ...
      expect(deltaNode.hasClass("delta-negative")).to.equal(true);
      expect(deltaNode.text()).to.be.equal("▲50 (100%)");
    });
    ...
    it("should render properly neutral delta", () => {
      ...
      expect(deltaNode.hasClass("delta-neutral")).to.equal(true);
    ...
    it("should handle infinite cases for delta percentage", () => {
      ...
      expect(positiveNode.hasClass("delta-positive")).to.equal(true);
      expect(positiveNode.text()).to.be.equal("▲100");

      expect(negativeNode.hasClass("delta-negative")).to.equal(true);
