...
export const BucketMarks: React.SFC<BucketMarksProps> = props => {
  const { stage, ticks, scale } = props;
  ...
  if (ticks.length) {
    const x = roundToHalfPx(scale(ticks[ticks.length - 1]) + scale.rangeBand());
    addLine(x, "last");
  }
