...
export class HighlightString extends React.Component<HighlightStringProps, HighlightStringState> {
  ...
  highlightInString(): any {
    ...
    return [
      JSX(`<span className="pre"  key="pre" >{text.substring(0, startIndex)}</span>`),
      JSX(`<span className="bold" key="bold">{text.substring(startIndex, endIndex)}</span>`),
      JSX(`<span className="post" key="post">{text.substring(endIndex)}</span>`)
    ];
  }
  ...
  render() {
    ...
    return JSX(`
      <div className={'highlight-string ' + (className || '')}>{this.highlightInString()}</div>
    `);
  }
}
