...
export class BarChart extends BaseVisualization<BarChartState> {
  ...
  onClick = (x: number, y: number) => {
    ...
    if (essence.highlightOn(series.get(chartIndex).measure.name)) {
    ...
    clicker.changeHighlight(series.get(chartIndex).measure.name, rowHighlight);
  }
  ...
  renderBars(
    ...
    data.forEach((d, i) => {
      ...
      if (splitIndex < splitLength - 1) {
        ...
        const selected = this.isSelected(subPath, series.measure);
  ...
  getSubCoordinates(
    ...
    const coordinates: BarCoordinates[] = data.map((d, i) => {
      ...
      if (splitIndex < essence.splits.length()) {
        ...
        const subData: Datum[] = (d[SPLIT] as Dataset).data;
        const subxScale = d3.scale.ordinal()
