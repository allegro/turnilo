...
import { fromConfig, MeasuresGroup } from "./measures";
import { MeasuresFixtures } from "./measures.fixtures";

describe("MeasureGroup", () => {
  it("should infer title from name", () => {
    const { tree } = fromConfig([MeasuresFixtures.noTitleJS()]);

    expect((tree[0] as MeasuresGroup).title).to.equal("Dummy Name");
  });

  it("should throw when no name given", () => {
    const measureGroupConversion = () => fromConfig([MeasuresFixtures.noNameJS()]);

    expect(measureGroupConversion).to.throw("measure group requires a name");
  });

  it("should throw when empty measures given", () => {
    const groupWithEmptyMeasures = MeasuresFixtures.emptyMeasuresJS();
    const measureGroupConversion = () => fromConfig([groupWithEmptyMeasures]);
