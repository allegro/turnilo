...
import { DimensionFixture } from "../../models/fixtures";
  ...
  describe("formatFilterClause", () => {
    ...
    latestDurationTests.forEach(({ duration, step, label }) => {
      it(`formats previous ${-step} * ${duration} as "${label}"`, () => {
        ...
        expect(formatFilterClause(DimensionFixture.time(), timeFilterLatest, Timezone.UTC)).to.equal(label);
      });
    });
    ...
    unsupportedLatestDurationTests.forEach(({ reference, duration, step }) => {
      it(`throws on formatting latest ${-step} * ${duration} with ${reference} reference"`, () => {
        ...
        expect(() => formatFilterClause(DimensionFixture.time(), timeFilterLatest, Timezone.UTC)).to.throw();
      });
    });
    ...
    durationTests.forEach(({ duration, previousLabel: label }) => {
      it(`formats previous ${duration} as "${label}"`, () => {
        ...
        expect(formatFilterClause(DimensionFixture.time(), timeFilterPrevious, Timezone.UTC)).to.equal(label);
      });
    });
    ...
    durationTests.forEach(({ duration, currentLabel: label }) => {
      it(`formats current ${duration} as "${label}"`, () => {
        ...
        expect(formatFilterClause(DimensionFixture.time(), timeFilterCurrent, Timezone.UTC)).to.equal(label);
      });
    });
    ...
    unsupportedPreviousDurationTests.forEach(({ reference, duration, step }) => {
      it(`throws on formatting previous ${-step} * ${duration} with ${reference} reference"`, () => {
        ...
        expect(() => formatFilterClause(DimensionFixture.time(), timeFilterEarlier, Timezone.UTC)).to.throw();
      });
    });
    ...
    unsupportedCurrentDurationTests.forEach(({ reference, duration }) => {
      it(`throws on formatting current ${duration} with ${reference} reference"`, () => {
        ...
        expect(() => formatFilterClause(DimensionFixture.time(), timeFilterCurrent, Timezone.UTC)).to.throw();
      });
    });
    ...
    fixedTimeTests.forEach(({ start, end, label }) => {
      it(`formats [${start}, ${end}) as "${label}"`, () => {
        ...
        expect(formatFilterClause(DimensionFixture.time(), filterClause, Timezone.UTC)).to.equal(label);
      });
    });
    ...
    fixedTimeTests.forEach(({ start, end, label }) => {
      it(`formats range [${start}, ${end}) as "time: ${label}"`, () => {
        const filterClause = FormatterFixtures.fixedTimeFilter(new Date(start), new Date(end));
        expect(formatFilterClause(DimensionFixture.time(), filterClause, Timezone.UTC, true)).to.equal(`time: ${label}`);
      });
    });

    it("formats number", () => {
      expect(formatFilterClause(DimensionFixture.number(), FormatterFixtures.numberFilter(), Timezone.UTC)).to.equal("Numeric (3)");
    });

    it("formats number verbose", () => {
      expect(formatFilterClause(DimensionFixture.number(), FormatterFixtures.numberFilter(), Timezone.UTC, true)).to.equal("Numeric: 1, 2, 3");
    });

    it("formats string", () => {
      expect(
        formatFilterClause(DimensionFixture.countryString(), FormatterFixtures.stringFilterShort(), Timezone.UTC)
      ).to.equal("important countries: iceland");
    });

    it("formats string verbose", () => {
      expect(
        formatFilterClause(DimensionFixture.countryString(), FormatterFixtures.stringFilterShort(), Timezone.UTC, true)
      ).to.equal("important countries: iceland");
    });
  });
});
