...
import { fromMeasure, SeriesDefinition } from "../../models/series/series-definition";
    ...
    return [
      {
        ...
        adjustment: {
          series: new SeriesList({ series: List(measures.map(measure => fromMeasure(measure))) })
        }
      }
    ];
  }
    ...
    return [
      {
        ...
        adjustment: {
          series: new SeriesList({ series: List.of(fromMeasure(firstMeasure)) })
        }
      }];
  }
}
