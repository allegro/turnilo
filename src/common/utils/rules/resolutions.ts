...
import { SeriesDefinition } from "../../models/series/series-definition";
    ...
    return [
      {
        ...
        adjustment: {
          series: new SeriesList({ series: List(measures.map(measure => SeriesDefinition.fromMeasure(measure))) })
        }
      }
    ];
  }
    ...
    return [
      {
        ...
        adjustment: {
          series: new SeriesList({ series: List.of(SeriesDefinition.fromMeasure(firstMeasure)) })
        }
      }];
  }
}
