...
import { EssenceFixture } from "../../models/essence/essence.fixture";
...
describe("urlHashConverter", () => {
  ...
  const tests: Array<{ version: ViewDefinitionVersion, hash: string, essence: Essence }> = [
    { version: "2", hash: UrlHashConverterFixtures.tableHashVersion2(), essence: EssenceFixture.wikiTable() },
    { version: "2", hash: UrlHashConverterFixtures.lineChartVersion2(), essence: EssenceFixture.wikiLineChart() },
    { version: "3", hash: UrlHashConverterFixtures.tableHashVersion3(), essence: EssenceFixture.wikiTable() },
    { version: "3", hash: UrlHashConverterFixtures.lineChartVersion3(), essence: EssenceFixture.wikiLineChart() }
  ];
  ...
  tests.forEach(({ version, hash, essence }) => {
    ...
    it(`decodes ${visualization.name} version ${version} correctly`, () => {
      const decodedEssence = urlHashConverter.essenceFromHash(hash, DataCubeFixture.wiki(), MANIFESTS);
    ...
    it(`is symmetric in decode/encode for ${visualization.name} in version ${version}`, () => {
      ...
      const decodedEssence = urlHashConverter.essenceFromHash(encodedHash, DataCubeFixture.wiki(), MANIFESTS);
  ...
  wrongHashStructureTests.forEach(({ hash, errorMessage }) => {
    it(`throws error for hash: "${hash}" with wrong structure`, () => {
      const essenceFromHashCall = () => urlHashConverter.essenceFromHash(hash, DataCubeFixture.wiki(), MANIFESTS);
