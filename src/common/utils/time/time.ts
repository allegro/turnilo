...
export function formatTimeRange(timeRange: TimeRange, timezone: Timezone, displayYear: DisplayYear): string {
  ...
  if (startWallTime.hour() || startWallTime.minute() || endWallTime.hour() || endWallTime.minute()) {
    ...
    let startTimeStr = startWallTime.format(FORMAT_TIME_OF_DAY_WITH_MINUTES).toLowerCase();
    const endTimeStr = endWallTime.format(FORMAT_TIME_OF_DAY_WITH_MINUTES).toLowerCase();
...
export function formatTimeBasedOnGranularity(range: TimeRange, granularity: Duration, timezone: Timezone, locale: Locale): string {
  ...
  const hour = pad(wallTimeStart.hour());
  const minute = pad(wallTimeStart.minute());
  const second = pad(wallTimeStart.second());
  ...
  switch (unit) {
    ...
    case "M":
      ...
      return prefix === "PT" ? `${monthString} ${day}, ${hour}:${minute}` : `${monthString}, ${year}`;
    case "H":
      return `${monthString} ${day}, ${year}, ${hour}:${minute}`;
    case "D":
