...
describe('cli', function () {
  ...
  it('shows help', (testComplete) => {
    exec('bin/swiv', (error, stdout, stderr) => {
  ...
  it('shows version', (testComplete) => {
    exec('bin/swiv --version', (error, stdout, stderr) => {
      expect(error).to.equal(null);
  ...
  it('prints the config', (testComplete) => {
    exec('bin/swiv --example wiki --print-config', (error, stdout, stderr) => {
      expect(error).to.equal(null);
      expect(stdout).to.contain('# generated by Swiv version');
  ...
  it('prints the config with comments', (testComplete) => {
    exec('bin/swiv --example wiki --print-config --with-comments', (error, stdout, stderr) => {
      expect(error).to.equal(null);
      expect(stdout).to.contain('# generated by Swiv version');
  ...
  it('reads a config an prints it inlined', (testComplete) => {
    exec('bin/swiv --config test/configs/inline-vars.yaml --print-config',
      {
  ...
  it('complains if there are too many settings inputs', (testComplete) => {
    exec('bin/swiv --example wiki --postgres localhost', (error, stdout, stderr) => {
      expect(error).to.be.an('error');
  ...
  it('complains if there are too many settings inputs (+message)', (testComplete) => {
    exec('bin/swiv --config blah.yaml --druid localhost', (error, stdout, stderr) => {
      expect(error).to.be.an('error');
      expect(stderr).to.contain('only one of --config, --examples, --file, --druid, --postgres, --mysql can be given on the command line');
